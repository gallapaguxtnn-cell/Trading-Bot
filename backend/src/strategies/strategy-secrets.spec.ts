...
describe('Strategy — segredos nunca retornados pelo GET /strategies', () => {
  ...
  it('legacyApiKey é select:false', () => {
    expect(col('legacyApiKey')?.options.select).toBe(false);
  });

  it('legacyApiSecret é select:false', () => {
    expect(col('legacyApiSecret')?.options.select).toBe(false);
  });
