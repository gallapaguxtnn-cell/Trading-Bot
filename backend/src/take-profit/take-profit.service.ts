...
export class TakeProfitService {
  ...
  private async checkTakeProfit(trade: Trade) {
    ...
    if (tp3 && this.shouldTrigger(trade, currentPrice, tp3)) {
      ...
      await this.closePosition(trade, strategy, currentPrice, 'TP3', 1.0);
      ...
      await this.closePosition(trade, strategy, currentPrice, 'TP2', 0.5);

      if (strategy.moveSLToBreakeven) {
        this.logger.log(`[MOVE SL TO BREAKEVEN] ${trade.symbol}`);
      }
      ...
      await this.closePosition(trade, strategy, currentPrice, 'TP1', 0.33);
    }
  }
