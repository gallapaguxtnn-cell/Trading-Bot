...
describe('BybitExchangeClient (adapter -- traduz AccountContext para o BybitClientService existente)', () => {
  ...
  beforeEach(() => {
    bybit = {
      createOrder: jest
        .fn()
        .mockResolvedValue({ orderId: 'o1', orderLinkId: 'l1' }),
      ...
      createStopLossOrder: jest
        .fn()
        .mockResolvedValue({ orderId: 'sl1', orderLinkId: 'l2' }),
      ...
      getSymbolRules: jest.fn().mockResolvedValue({
        qtyStep: '0.001',
        priceTick: '0.01',
        minQty: '0.001',
        minNotional: '5',
  ...
  it('mode REAL -> isTestnet false; mode DEMO -> isTestnet true (createOrder)', async () => {
    await client.createOrder(makeCtx({ mode: 'REAL' }), {
      symbol: 'BTCUSDT',
      side: 'BUY',
      orderType: 'MARKET',
      qty: '1',
    ...
    await client.createOrder(makeCtx({ mode: 'DEMO' }), {
      symbol: 'BTCUSDT',
      side: 'SELL',
      orderType: 'LIMIT',
      qty: '1',
  ...
  it('region e repassado como siteId em toda chamada autenticada', async () => {
    ...
    expect(bybit.getPositions).toHaveBeenCalledWith(
      'key',
      'secret',
      false,
      'BTCUSDT',
    ...
    expect(bybit.cancelOrder).toHaveBeenCalledWith(
      'key',
      'secret',
      false,
      'BTCUSDT',
  ...
  it('createStopLossOrder, setTradingStop, clearTradingStop, getPositionIdx e waitForPosition traduzem side neutro para Buy/Sell', async () => {
    const ctx = makeCtx();
    await client.createStopLossOrder(
      ctx,
      'BTCUSDT',
      'SELL',
      '1',
    ...
    expect(bybit.createStopLossOrder).toHaveBeenCalledWith(
      'key',
      'secret',
      false,
      'BTCUSDT',
    ...
    expect(bybit.setTradingStop).toHaveBeenCalledWith(
      'key',
      'secret',
      false,
      'BTCUSDT',
    ...
    expect(bybit.clearTradingStop).toHaveBeenCalledWith(
      'key',
      'secret',
      false,
      'BTCUSDT',
    ...
    expect(bybit.getPositionIdx).toHaveBeenCalledWith(
      'key',
      'secret',
      false,
      'BTCUSDT',
    ...
    expect(bybit.waitForPosition).toHaveBeenCalledWith(
      'key',
      'secret',
      false,
      'BTCUSDT',
  ...
  it('setLeverage, setMarginMode, getWalletBalance, getLastTradePrice, getOpenOrders, getOrderInfo, getOrderHistory, cancelAllOrders, detectPositionMode repassam credenciais/isTestnet/siteId corretamente', async () => {
    ...
    expect(bybit.setLeverage).toHaveBeenCalledWith(
      'key',
      'secret',
      false,
      'BTCUSDT',
    ...
    expect(bybit.setMarginMode).toHaveBeenCalledWith(
      'key',
      'secret',
      false,
      'BTCUSDT',
    ...
    expect(bybit.getWalletBalance).toHaveBeenCalledWith(
      'key',
      'secret',
      false,
      'ARG_BTL',
    ...
    expect(bybit.getLastTradePrice).toHaveBeenCalledWith(
      'key',
      'secret',
      false,
      'BTCUSDT',
    ...
    expect(bybit.getOpenOrders).toHaveBeenCalledWith(
      'key',
      'secret',
      false,
      'BTCUSDT',
    ...
    expect(bybit.getOrderInfo).toHaveBeenCalledWith(
      'key',
      'secret',
      false,
      'BTCUSDT',
    ...
    expect(bybit.getOrderHistory).toHaveBeenCalledWith(
      'key',
      'secret',
      false,
      'BTCUSDT',
    ...
    expect(bybit.cancelAllOrders).toHaveBeenCalledWith(
      'key',
      'secret',
      false,
      'BTCUSDT',
    ...
    expect(bybit.detectPositionMode).toHaveBeenCalledWith(
      'key',
      'secret',
      false,
      'BTCUSDT',
  ...
  it('ensurePositionMode e no-op para Bybit -- a corretora detecta o modo automaticamente via positionIdx', async () => {
    await expect(
      client.ensurePositionMode(makeCtx(), 'BTCUSDT', true),
    ).resolves.toBeUndefined();
    expect(
      Object.values(bybit).every(
